...
class DockerVSCodePHPCSFixer {
    ...
    format(text) {
        autoFixing = true;
        let tmpFileName = 'temp-' + Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 10) + '.php';
        let tmpFile = TmpDir + '/' + tmpFileName;
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            exec.on("exit", (code) => {
                if (code == 0) {
                    let fixed = fs.readFileSync(tmpFile, 'utf-8');
                    if (fixed.length > 0) {
                        resolve(fixed);
                    } else {
                        reject();
                    ...
                    let msgs = {
                        1:  'PHP CS Fixer: php general error. ' + fileName,
...
exports.activate = (context) => {
    let pcf = new DockerVSCodePHPCSFixer();
